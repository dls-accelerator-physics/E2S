...
###Crystal
code=Silicon   # works with coway=0 only
###Other material:
...
detail=1	# 0 - don't print coords, 1 = print coords
###-----------------------------------------------------------
...
{
####--------Print header------------------------
#echo "  Energy [kev],               xr0 [psi0r],    xi0 [psi0i],      xrhsg [psi_hr]   ,         xihsg [psi_hi] ,   psi_hbr     psi_hbi       //not printed:d [d_sp]"

###--------Loop over energy--------------------
#for ((i=0; i < $n; i++))	# loop over energy points
# take just the first point
for ((i=0; i < 1; i++))	# loop over energy points
...
#  x=(`curl  --silent $address | sed -n '/xr0=/{s/.*xr0=//;p;};/xi0=/{s/.*xi0=//;p;};/QB=/{s/.*QB=//;p;};/d=/{s/.*d=//;p;};/xrhsg=/{s/.*xrhsg=//;p;};/xihsg=/{s/.*xihsg=//;p;}'`)
  x=(`curl  --silent $address | sed -n '/xr0=/{s/.*xr0=//;p;};/xi0=/{s/.*xi0=//;p;};/xrhsg=/{s/.*xrhsg=//;p;};/xihsg=/{s/.*xihsg=//;p;}'`)

#echo $x   n'imprimera que le premier element

###--------Print current point-------------------
  echo "   ${wave}     ${x[0]}    ${x[1]}           ${x[2]}          ${x[3]}         ${x[2]}         ${x[3]} "
